	...
	<script>
		...
		layui.use('table', function() {
			...
			table.render({
				elem : '#demo' // 绑定控件
				,
				url : 'GoodsServlet?op=serach&state=1' // 数据接口
				,
				cols : [ [ // 表头
				{
					field : 'goodName',
		...
		function del(This) {
			layui.use('table', function() {
				...
				table.on('tool(test)', function(obj) { // test是table的属性 lay-filter="test"
					...
					$.post("GoodsServlet", "op=updState&id=" + id + "&state=2",
							function(data) {
								if (data == "1") {
									layer.msg("删除成功！");
									// 模拟点击
		...
		function upd() {
			layui.use('table', function() {
				...
				table.on('tool(test)', function(obj) { // test是table的属性 lay-filter="test"
					var data = obj.data; // 获得当前行数据
					var id = data['id']; // 获取属性id的值
					$.post("GoodsServlet", "op=toupd&id=" + id, function(data) {
						if (data == "ok") {
							layer.open({
								type : 2,
								shade : 0.1, // 遮罩层透明度
								...
								cancel : function(index, layero) {
									...
									$(".layui-laypage-btn").click();
									// 验证信号是否存在
									if (sessionStorage.getItem("goodsUpd") == "1") {
										layer.msg("修改成功！");
										// 清除信号
										sessionStorage.removeItem("goodsUpd");
									}
								}
							});
						} else {
		...
		function add() {
			layui.use('table', function() {
				...
				var table = layui.table;
				// 监听
				$.post("GoodsServlet", "op=toadd", function(data) {
					if (data == "ok") {
						layer.open({
							...
							cancel : function(index, layero) {
								...
								$(".layui-laypage-btn").click();
								// 验证信号是否存在
								if (sessionStorage.getItem("goodsAdd") == "1") {
									layer.msg("新增成功！");
									// 清除信号
									sessionStorage.removeItem("goodsAdd");
								}
							}
						});
					} else {
