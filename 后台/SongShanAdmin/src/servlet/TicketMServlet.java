...
public class TicketMServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		TicketMDaoImpl tdmi = new TicketMDaoImpl();
		...
		if (op == null || op.equals("serach")) {
			...
			List<TicketM> arr = tdmi.arrSell(page, limit, state);
			// 返回参数对象
			...
			user = user == null ? "" : user;
			List<TicketM> arr = tdmi.arrSell(page, limit, state, goodName, user);
			// 返回参数对象
			TableCode tc = new TableCode(0,tdi.sum(state),"",arr);
