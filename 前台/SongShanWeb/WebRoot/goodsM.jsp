		...
		<div class="shop_list block">
			<ul class="list_pic_rank ani block">
				...
				<c:forEach items="${arr}" var="a">
					...
					<li class="ticket">
						<a href="WebGoodsMServlet?op=desc&id=${a.id}" title="${a.goodName}">
							<img src="${a.imgSrc}" alt="${a.goodName}">
						</a>
						<h3>
							<a href="WebGoodsMServlet?op=desc&id=${a.id}" title="${a.goodName}">${a.goodName}</a>
						</h3>
						<p>购买后5日内有效，可使用一次</p>
						<h5 class="en"><small>￥</small>${a.money}</h5>
						<a href="WebGoodsMServlet?op=desc&id=${a.id}" title="查看详情" class="more">查看详情</a>
					</li>
